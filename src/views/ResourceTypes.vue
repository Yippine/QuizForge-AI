...
const { loading, error, loadResourcesMap, getCertificationById, getSubjectById, getCategoryById, getLevelById } = useResourcesMap()
  ...
  <div class="min-h-screen bg-gradient-to-br from-primary-50 via-white to-secondary-50 py-12 px-4">
    ...
    <div v-if="loading" class="flex items-center justify-center min-h-screen">
      <div class="text-center">
        ...
        <p class="text-gray-600 text-lg">載入資源類型...</p>
      </div>
    </div>
    ...
    <div v-else-if="error" class="max-w-2xl mx-auto">
      <div class="bg-red-50 border border-red-200 rounded-xl p-6 text-center">
        ...
        <button @click="goToSubjectHub" class="bg-red-600 text-white px-6 py-2 rounded-lg hover:bg-red-700 transition-colors">
          返回科目選擇
        </button>
      </div>
    </div>
    ...
    <!-- Main Content -->
    <div v-else-if="subject && certification" class="max-w-6xl mx-auto">
      <!-- Header -->
      <div class="mb-8">
        <div class="flex items-center gap-4 mb-6">
          <button v-if="isNewStructure" @click="goToHome" class="flex items-center gap-2 text-gray-600 hover:text-primary-600 transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
            </svg>
            返回首頁
          </button>
          <span v-if="isNewStructure" class="text-gray-300">|</span>
          <button @click="goToSubjectHub" class="flex items-center gap-2 text-gray-500 hover:text-primary-600 transition-colors text-sm">
            <svg v-if="!isNewStructure" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
            </svg>
        ...
        <div class="text-center">
          ...
          <div v-if="!isNewStructure" class="flex items-center justify-center gap-2 text-sm text-gray-600 mb-4">
          ...
          <div v-else class="flex items-center justify-center gap-2 text-sm text-gray-600 mb-4 flex-wrap">
            ...
            <span class="hover:text-primary-600 cursor-pointer" @click="goBack">{{ subject.code }}</span>
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
            </svg>
          ...
          <h1 class="text-4xl font-bold text-gray-900 mb-4">{{ isNewStructure ? '講義資源' : '選擇資源類型' }}</h1>
        ...
        >
          <div class="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p class="text-sm text-gray-500">{{ resourceTypeMetadata[resource.type]?.description || resource.description }}</p>
            </div>
          </div>
